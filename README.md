...
└── seoul_elderly_population_grid.shp
```
...
| `Data/seoul_elderly_population_grid.shp` | 국토정보플랫폼 서울시 고령인구 격자 데이터 |
...
| `고령인구_300m` | 300m 반경 내 65세 이상 거주인구 |
...
4. 데이터결합.ipynb 실행
5. Data/seoul_road_points.csv 확인
```
...
```text
POINT_ID, 위도, 경도, 제한속도, 위험도, 고령인구_300m, 상가 업종별 개수 컬럼 85개
```
...
| `고령인구_300m` | 국토정보플랫폼 격자 인구 데이터 기준 300m 반경 내 65세 이상 거주인구 |
...
```text
Data/seoul_elderly_population_grid.shp
```

또는:

```text
Data/국토정보플랫폼_서울_고령인구_격자.shp
```

사용 내용:

- 국토정보플랫폼에서 서울시 격자 기반 인구 데이터를 내려받습니다.
- 65세 이상 거주인구 컬럼이 필요합니다.
- `shp`, `geojson`, `gpkg`, `csv` 형식을 지원합니다.
- CSV는 `geometry` WKT 컬럼 또는 `x/y`, `경도/위도` 좌표 컬럼이 필요합니다.

처리 방식:

- 데이터를 도로 포인트 좌표계로 변환합니다.
- 격자 polygon 데이터는 격자 대표점 기준으로 300m 버퍼 안 포함 여부를 판단합니다.
- 각 `POINT_ID` 기준 300m 반경 안에 포함된 격자의 65세 이상 인구를 합산해 `고령인구_300m` 컬럼으로 추가합니다.
